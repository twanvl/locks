...
waferC = C;
tightC = 0.08;
bridgeC = roundToLayerHeight(0.5); // clearance for bridges
...
//waferThickness = roundToLayerHeight(1.5);
waferThicknessLast = roundToLayerHeight(4);
waferStep = waferThickness;
waferWidth = 11;
waferLip = 0.6;
tabWidth = 8;
tabSlope = 0.3;
...
faceR = coreR+1;
faceThickness = waferThickness;
faceCountersink = waferThickness;
faceChamfer = 0;
faceAlignerThickness = waferThickness;
waferPos = 0;//faceThickness + faceAlignerThickness;
...
keyC = 0.3; // vertical clearance in keyhole
keyCX = C; // horizontal clearance in keyhole (not critical)
...
clipSep = 0.8;
coreBackOverlap = 1;
coreStackDepth = wafers * waferStep + waferThicknessLast + bridgeC - coreBackOverlap;
...
housingRX = housingRY;
housingChamfer = 1;
coreDepth = faceThickness + faceAlignerThickness - faceCountersink + coreStackDepth;
housingDepth = coreDepth + coreBack + clipSep;

housingChamfer2 = housingChamfer*0.5; // chamfer inside cap
capEdge = 1.6;
capCountersink = 1.6;
//capDepth = capEdge+capCountersink+1+6;
capDepth = 2*housingRY - housingDepth + capCountersink;
capClip = 1;
capClipLength = 10;
echo ("housing",2*housingRX,2*housingRY,housingDepth+capDepth-capCountersink);
...
module key_profile(C=0) {
  w = keyWidth - keyHeight;
  linear_extrude(eps) {
    translate_y(keyDelta/2)
    //line([[-keyWidth/2,-keyDelta],[0,0],[keyWidth/2,-keyDelta]],keyHeight/2);
    line([[-w/2,-keyDelta],[-0.3,0],[0.3,0],[w/2,-keyDelta]],keyHeight+C);
  }
}
*!key_profile();

module extrude_key(C=0) {
...
module key_hole() {
  ...
  extrude_key(C) {
    sym_polygon_x([
      [keyDC+toothSlope*l, keyA-l],
      [keyDC+eps, keyA],
      [keyDC+eps, keyB],
      [keyDC+toothSlope*l, keyB+l],
    ]);
  }
}
...
module tab_profile() {
  rotated(180) translate_y(1) square([waferWidth/2,lots]);
}
//!linear_extrude(waferThickness,true) tab_profile();
//connectorR=2;connectorO=[0,0,-0.5];

module connector_profile2(C=0,z=0) {
  pinW = 1.2;
  pinH = roundToLayerHeight(0.0);
  pinH2 = roundToLayerHeight(min(pinW,0.6 + 100*C));
  polygon([
    [waferWidth/2-pinW-C,z-layerHeight],
    [waferWidth/2-pinW-C,z+pinH],
    [waferWidth/2-pinW+pinH2,z+C+pinH+pinH2],
    [waferWidth/2+eps,z+C+pinH+pinH2],
    [waferWidth/2+eps,z-layerHeight]]);
}
  ...
  difference() {
    union() {
      ...
      if (tabs) intersection() {
        ...
        cylinder(r=coreR,h=thickness-layerHeight);
      }
      rotated(180) translate_y(coreR-0.4) cylinder(r=0.4,h=thickness-layerHeight);
      // profile
      intersection() {
        *wafer_profile();
        cube([waferWidth,lots,lots],true);
        translate([0,y-maxBit*step,0]) cylinder(r=coreR,h=thickness-layerHeight);
        translate([0,y-minBit*step,0]) cylinder(r=coreR,h=thickness-layerHeight);
      }
      // friction surface
      translate([0,y,thickness-layerHeight-eps]) {
        minkowski() {
          key_profile(C=C+0.4);
          h = (thickness-keyB)*toothSlope + keyC-C;
          translate_y(-h)
          cube([eps,eps+2*h,layerHeight+eps]);
        }
      }
      // connector
      if (pin) {
        intersection() {
          ...
          rotated(180)
          //translate_y(-lots)
          translate_y(connectorY+maxDelta)
          linear_extrude_y(lots) {
            connector_profile2(z=thickness-eps);
          }
          cylinder(r=coreR,h=lots);
        }
      }
      *if (pin) {
        intersection() {
    ...
    *if (slot) {
    ...
    if (slot) {
      translate_y(y)
      rotated(180)
      translate_y(connectorY)
      linear_extrude_y(lots) {
        connector_profile2(C=C,z=-eps);
      }
    }
  }
...
module first_wafer2(bit=0) {
  b = maxBit-1;
  intersection() {
    wafer(-1,-b,b,slot=false);
    wafer(0,-b,b,slot=false);
    wafer(1,-b,b,slot=false);
  }
}
...
module waferTest() {
  intersection() {
    group() {
      wafer(minBit);
      color("lightgreen") translate([0,-0*step,2+C]) wafer(minBit+2);
...
module waferKeyTest() {
  ...
  for (i=[0:len(bitting)-1]) {
    jr = i - z/waferStep;
    j1 = max(0,min(len(bitting)-1,floor(jr)));
    j2 = max(0,min(len(bitting)-1,ceil(jr)));
    jd = jr-j1;
    x = (1-jd)*(bitting[j1]-bitting[i])*step + jd*(bitting[j2]-bitting[i])*step;
    color(i%2 ? "pink" : "lightgreen") translate([i*eps,x,i*waferStep]) 
    intersection() {
      wafer(bitting[i]);
      cube([0.1,lots,lots],true);
    }
  }
  color("salmon")
  translate([-eps,0,z]) 
  intersection() {
    extrude_key() {
      line([for (i=[0:len(bitting)-1]) each
          [[bitting[i]*step,i*waferStep+keyA],[bitting[i]*step,i*waferStep+keyB]]],eps);
    }
    cube([0.1,lots,lots],true);
  }
}
...
module core(C=C,CX=C) {
  union() {
    ...
    intersection() {
      difference() {
        union() {
          cylinder(r=coreR,h=coreStackDepth);
          translate_z(coreStackDepth) cylinder(r1=coreR,r2=clipCoreR,h=coreBack);
          translate_z(coreStackDepth+coreBack) cylinder(r=clipCoreR,h=clipH+clipSep+C);
        }
        // slots for wafers
        translate([0,0,-eps])
        wafer_profiles(C=layerHeight,CX=CX);
        // chamfer for wafer slots
        ww = waferWidth/2 + waferLip + CX;
        h = wafers * waferStep + C-2*eps;
        p = on_circle(coreR,ww) + [0,-0.25];// where wafer slots end
        chamfer = 1;
        rotated(180) linear_extrude(h) {
          sym_polygon_x([p+[5,5],p+[5,0],p,p+[-chamfer,-chamfer/2]]);
        }
        /*
        ...
        */
        ...
        translate_z(coreStackDepth+coreBack+clipSep+C)
...
module housing() {
  ...
  difference() {
    ...
    translate([0,0,-eps]) cylinder(r=coreR+C,h=coreDepth+2*eps);
    translate([0,0,coreDepth]) cylinder(r1=coreR+C,r2=clipCoreR+C,h=coreBack);
    translate([0,0,coreDepth+coreBack]) cylinder(r=clipCoreR+C,h=coreBack);
    // face
...
module outer_housing() {
  translate_z(-waferThickness)
  linear_extrude(waferThickness) {
    difference() {
      square(2*(coreR+2),true);
      // triangle with x = keyWidth/2, y=keyHeight/2+maxBit*step
      w = diagonal(keyWidth/2,keyHeight/2+maxBit*step);
      circle(w+C);
    }
  }
}
...
module test() {
  ...
  intersection() {
    group() {
      if (showHousing) translate([0,0,0]) color("lightyellow") intersection() {
        if (exposed) exposed_housing(); else outer_housing();
        //if (cutHousing!=undef) translate_y(cutHousing) positive_y();
      }
      if (showHousing) translate([0,0,0]) intersection() {
        color("white") if (exposed) exposed_housing(); else housing();
        if (cutHousing!=undef) translate_y(cutHousing) positive_y();
      }
      *if (showHousing) translate([0,0,1+housingDepth-capCountersink+tightC]) intersection() {
        color("lightblue") housing_back();
        if (cutHousing!=undef) translate_y(cutHousing) positive_y();
      }
      rotate(0) {
        if (showCore) intersection() {
          color("pink") translate([0,0,corePos]) core();
          if (cutHousing!=undef) rotate(cutCoreA) translate_y(cutHousing) positive_y();
        }
        if (showWafers) {
          //translate([0,0,0]) translate_y(maxBit*step) color("lightgreen") wafer(0);
          //translate([0,0,0]) translate_y(0*step) color("lightgreen") wafer(maxBit);
          translate([0,0,0]) translate_y((maxBit-1)*step) color("lightgreen") first_wafer2();
          translate([0,0,0+waferStep+layerHeight/2]) color("lightblue") wafer(0);
          translate([0,0,0+wafers*waferStep+layerHeight/2]) color("lightyellow") last_wafer();
        }
        *translate([0,0,faceThickness+coreStackDepth+coreBack+clipSep+C+tightC/2]) rotate(90) color("lightgreen") retaining_clip();
      }
    }
    //positive_y();
...
module labled_core(CX,label) {
  ...
  translate([0,0,faceThickness + coreStackDepth + coreBack + clipSep + clipH]) linear_extrude(0.2)
